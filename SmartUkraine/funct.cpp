...
int Autefication()
{
	...
	while (ch != 13)
	{
		...
		case 224:
		{
			switch (_getch())
			{
			case 80: {counter < 5 ? counter++ : counter = 1; break; }
			case 72: {counter > 1 ? counter-- : counter = 5; break; }
			}
		}
		}
	}
...
string GetPath() 
{
	...
	char* b = const_cast<char*>(filename.c_str());

	char* pointer = const_cast<char*>(strstr(b, "SmartUkraine"));
	int i = 0;
	while (pointer[i] != '\\') 
	{
		i++;
